...
**`build_surface`** runs the solver across a grid of expiries and strikes, returning a 2D array where rows are expiries and columns are strikes.

**`interpolate_iv`** queries IV at any arbitrary target `(T, K)` point using bilinear interpolation — bridging the gap between discrete market quotes and continuous hedging needs.
...
### `build_surface(prices, S, expiries, strikes, r, flag)`
...
Returns interpolated IV at target `(T, K)`. Raises `ValueError` if target is outside the grid.
